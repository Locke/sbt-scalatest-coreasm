...
"""
  ...
  def apply(packageName: String, srcDirIn: File, filesIn: Set[File], outDirIn: File, log: Logger): Set[File] = {
    ...
    val generatedFiles: Set[File] = testNamesWithFiles.map(x => {
        val testName = x._1
        val files = x._2
        generate(packageName, files, outDir, testName)
      }).toSet.flatten
  ...
  private def generate(packageName: String, files: Set[File], outDir: File, testName: String): Set[File] = {
    val filesAbsolute = files.map(f => "new JFile(\"\"\""+f.getCanonicalPath+"\"\"\")")

    val filesSeq = filesAbsolute.mkString("Seq(", ", ", ").map(_.toPath)")

    val source = template format (packageName, info, "Test" + testName, filesSeq)
